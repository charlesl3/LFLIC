...
%% 
% convergence plot: absolute cosine similarity between one tracked neural
% weight vector and the leading PC of the input used in each AIME cycle.
% Because AIME components are axes rather than oriented vectors, the
% absolute cosine similarity is reported.

figure;
tiledlayout(2,2,'TileSpacing','compact','Padding','compact');

nexttile;
plot(1:size(training_dot,1), abs(training_dot(:,1)), 'LineWidth', 1.5);
xlabel('Epoch');
ylabel('|cosine similarity|');
title('Cycle 1');
ylim([0 1]);
xlim([1 size(training_dot,1)]);
grid on;
set(gca, 'FontSize', 11);
...
nexttile;
plot(1:size(training_dot2,1), abs(training_dot2(:,1)), 'LineWidth', 1.5);
xlabel('Epoch');
ylabel('|cosine similarity|');
title('Cycle 2');
ylim([0 1]);
xlim([1 size(training_dot2,1)]);
grid on;
set(gca, 'FontSize', 11);
set(gca, 'LineWidth', 1.0);
set(gca, 'FontName', 'Times New Roman');
...
nexttile;
plot(1:size(training_dot3,1), abs(training_dot3(:,1)), 'LineWidth', 1.5);
xlabel('Epoch');
ylabel('|cosine similarity|');
title('Cycle 3');
ylim([0 1]);
xlim([1 size(training_dot3,1)]);
grid on;
set(gca, 'FontSize', 11);
set(gca, 'LineWidth', 1.0);
set(gca, 'FontName', 'Times New Roman');
...
nexttile;
plot(1:size(training_dot4,1), abs(training_dot4(:,1)), 'LineWidth', 1.5);
xlabel('Epoch');
ylabel('|cosine similarity|');
title('Cycle 4');
ylim([0 1]);
xlim([1 size(training_dot4,1)]);
grid on;
set(gca, 'FontSize', 11);
set(gca, 'LineWidth', 1.0);
set(gca, 'FontName', 'Times New Roman');

sgtitle('AIME convergence across learning cycles - Iris');
