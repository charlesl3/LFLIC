%% AIME synthetic-data simulation
...
for e = 1:epoch;
    ...
    for col = 1:size(sampled_data,1); % loop over all inputs
     ...
     % Post-hoc-compatible trajectory:
     % center the population exactly as in the final line-identification step.
...
% Representative neural weight for Line11:
% choose the final learned vector most aligned with the post-hoc centroid.
line11_dir = normr(C(1,:));
line11_members = find(idx == 1);
[~,line11_local_id] = max(abs(new_weight(line11_members,:)*line11_dir'));
line11_rep_idx = line11_members(line11_local_id);
conv_line11_pc1 = pc1_alignment_history(:,line11_rep_idx);
...
pc2_alignment_history = zeros(epoch,n_dst);
...
for e = 1:epoch;
    ...
    if e>epoch*0.5;
     ...
     epoch_centered_weights = ori_cycle2_cells-mean(ori_cycle2_cells,2);
     epoch_centered_weights = normc(epoch_centered_weights);
     pc2_alignment_history(e,:) = abs(epoch_centered_weights'*normc(coeff_real(:,2)))';
end
...
line21_dir = normr(C(2,:));
line21_members = find(idx == 2);
[~,line21_local_id] = max(abs(new_weight(line21_members,:)*line21_dir'));
line21_rep_idx = line21_members(line21_local_id);
conv_line21_pc2 = pc2_alignment_history(:,line21_rep_idx);
...
pc3_alignment_history = zeros(epoch,n_dst);
...
for e = 1:epoch;
    ...
    if e>epoch*0.5;
     ...
     epoch_centered_weights = ori_cycle3_cells-mean(ori_cycle3_cells,2);
     epoch_centered_weights = normc(epoch_centered_weights);
     pc3_alignment_history(e,:) = abs(epoch_centered_weights'*normc(coeff_real(:,3)))';
end
...
% Representative neural weights for the two post-hoc Cycle-3 lines.
line31_members = find(idx == 2);
[~,line31_local_id] = max(abs(new_weight(line31_members,:)*c3_line1'));
line31_rep_idx = line31_members(line31_local_id);

line32_members = find(idx == 4);
[~,line32_local_id] = max(abs(new_weight(line32_members,:)*c3_line2'));
line32_rep_idx = line32_members(line32_local_id);

conv_line31_pc3 = pc3_alignment_history(:,line31_rep_idx);
conv_line32_pc3 = pc3_alignment_history(:,line32_rep_idx);
...
pc4_alignment_history_branch1 = zeros(epoch,n_dst);
...
for e = 1:epoch;
    ...
    epoch_centered_weights = ori_cycle4_cells-mean(ori_cycle4_cells,2);
    epoch_centered_weights = normc(epoch_centered_weights);
    pc4_alignment_history_branch1(e,:) = abs(epoch_centered_weights'*normc(coeff_real(:,4)))';
end
...
% Representative neural weights for Line41 and Line42.
line41_members = find(idx == 3);
[~,line41_local_id] = max(abs(new_weight(line41_members,:)*c4_line1'));
line41_rep_idx = line41_members(line41_local_id);

line42_members = find(idx == 4);
[~,line42_local_id] = max(abs(new_weight(line42_members,:)*c4_line2'));
line42_rep_idx = line42_members(line42_local_id);

conv_line41_pc4 = pc4_alignment_history_branch1(:,line41_rep_idx);
conv_line42_pc4 = pc4_alignment_history_branch1(:,line42_rep_idx);
...
% cycle 4, find Line43 (inheritance from Line32)
cycle = 4;
ori_cycle4_cells = cells;
final_weight = [];
epoch = 400;

pc4_alignment_history_branch2 = zeros(epoch,n_dst);
...
for e = 1:epoch;
    ...
    epoch_centered_weights = ori_cycle4_cells-mean(ori_cycle4_cells,2);
    epoch_centered_weights = normc(epoch_centered_weights);
    pc4_alignment_history_branch2(e,:) = abs(epoch_centered_weights'*normc(coeff_real(:,4)))';
end
...
% Representative neural weight for Line43.
line43_members = find(idx == 1);
[~,line43_local_id] = max(abs(new_weight(line43_members,:)*c4_line3'));
line43_rep_idx = line43_members(line43_local_id);
conv_line43_pc4 = pc4_alignment_history_branch2(:,line43_rep_idx);
...
%% Post-hoc line convergence visualization
% IMPORTANT:
% AIME lines are defined only after training by K-means on the final learned
% weight population. Therefore, a line itself cannot be tracked during
% training. For each final post-hoc line, we select one representative
% neural weight vector from the corresponding final K-means group (the
% member most aligned with that group's centroid), and retrospectively plot
% that same neuron's epoch-by-epoch alignment with the corresponding true PC.
%
% These curves are therefore representative approximations to the emergence
% of each final AIME line, not trajectories of the post-hoc centroids themselves.
...
% PC1 versus Line11
nexttile;
plot(1:epoch,conv_line11_pc1,'LineWidth',1.5);
xlabel('Epoch');
ylabel('|cosine similarity|');
title('PC1 and Line_{11}');
ylim([0 1]);
xlim([1 100]);
grid on;
...
% PC2 versus Line21
nexttile;
plot(1:epoch,conv_line21_pc2,'LineWidth',1.5);
xlabel('Epoch');
ylabel('|cosine similarity|');
title('PC2 and Line_{21}');
ylim([0 1]);
xlim([1 100]);
grid on;
set(gca,'FontSize',11,'LineWidth',1.0,'FontName','Times New Roman');

% PC3 versus the two Cycle-3 AIME branches
nexttile;
plot(1:epoch,conv_line31_pc3,'LineWidth',1.5);
hold on;
plot(1:epoch,conv_line32_pc3,'LineWidth',1.5);
hold off;
xlabel('Epoch');
ylabel('|cosine similarity|');
title('PC3 and Cycle-3 AIME lines');
legend('Line_{31}','Line_{32}','Location','best');
ylim([0 1]);
xlim([1 100]);
grid on;
set(gca,'FontSize',11,'LineWidth',1.0,'FontName','Times New Roman');

% PC4 versus the three Cycle-4 AIME branches
nexttile;
plot(1:epoch,conv_line41_pc4,'LineWidth',1.5);
hold on;
plot(1:epoch,conv_line42_pc4,'LineWidth',1.5);
plot(1:epoch,conv_line43_pc4,'LineWidth',1.5);
hold off;
xlabel('Epoch');
ylabel('|cosine similarity|');
title('PC4 and Cycle-4 AIME lines');
legend('Line_{41}','Line_{42}','Line_{43}','Location','best');
ylim([0 1]);
xlim([1 100]);
grid on;
set(gca,'FontSize',11,'LineWidth',1.0,'FontName','Times New Roman');

sgtitle('AIME convergence across learning cycles - Synthetic');
